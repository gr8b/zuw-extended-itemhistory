...
namespace Modules\ExtendedItemHistory\Includes;
...
class CWidgetFieldColumnsList extends CWidgetField {
	...
	public const DISPLAY_TEXT_EXTENDED = 6;
	...
	public function toApi(array &$widget_fields = []): void {
		...
		foreach ($this->getValue() as $column_index => $value) {
			...
			if (array_key_exists('regexes', $value) && $value['regexes']) {
				foreach ($value['regexes'] as $regex_index => $regex) {
					$widget_fields[] = [
						'type' => ZBX_WIDGET_FIELD_TYPE_STR,
						'name' => $this->name.'.'.$column_index.'.regexes.'.$regex_index.'.match',
						'value' => $regex['match']
					];

					$widget_fields[] = [
						'type' => ZBX_WIDGET_FIELD_TYPE_STR,
						'name' =>  $this->name.'.'.$column_index.'.regexes.'.$regex_index.'.replace',
						'value' => $regex['replace']
					];
				}
			}
	...
	protected function getValidationRules(bool $strict = false): array {
		$validation_rules = ['type' => API_OBJECTS, 'fields' => [
			...
			'display'			=> ['type' => API_INT32, 'default' => self::DISPLAY_AS_IS, 'in' => implode(',', [self::DISPLAY_AS_IS, self::DISPLAY_BAR, self::DISPLAY_INDICATORS, self::DISPLAY_HTML, self::DISPLAY_SINGLE_LINE, self::DISPLAY_TEXT_EXTENDED])],
			...
			'regexes'			=> ['type' => API_OBJECTS, 'fields' => [
				'match'				=> ['type' => API_REGEX, 'flags' => API_NOT_EMPTY, 'length' => 255],
				'replace'			=> ['type' => API_STRING_UTF8, 'flags' => API_REQUIRED | API_NOT_EMPTY, 'length' => 255]
			]],
