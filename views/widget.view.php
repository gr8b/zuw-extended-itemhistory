...
use Modules\ExtendedItemHistory\Widget;

use Modules\ExtendedItemHistory\Includes\{
	WidgetForm,
	...
	switch ($column['item_value_type']) {
		...
		case ITEM_VALUE_TYPE_TEXT:
			...
			switch ($column['display']) {
				...
				case CWidgetFieldColumnsList::DISPLAY_TEXT_EXTENDED:
					$value = htmlspecialchars($item_value['value'], ENT_NOQUOTES);

					foreach ($column['regexes'] as $regex) {
						$value = preg_replace('/'.$regex['match'].'/', $regex['replace'], $value);
					}

					$cell->addItem(new CJsScript($value));
