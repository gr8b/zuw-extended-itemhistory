...
window.widget_extendeditemhistory_form = new class {
	...
	#processColumnsAction(e) {
		...
		switch (target.getAttribute('name')) {
			case 'add':
				...
				column_popup = PopUp(
					'widget.extendeditemhistory.column.edit',
	...
	#updateColumns(e) {
		...
		if (data.regexes) {
			for (const [key, value] of Object.entries(data.regexes)) {
				input.setAttribute('name', `columns[${this.#column_index}][regexes][${key}][match]`);
				input.setAttribute('value', value.match);
				this.#form.appendChild(input.cloneNode());
				input.setAttribute('name', `columns[${this.#column_index}][regexes][${key}][replace]`);
				input.setAttribute('value', value.replace);
				this.#form.appendChild(input.cloneNode());
			}
