...
use Modules\ExtendedItemHistory\Includes\CWidgetFieldColumnsList;
...
window.item_history_column_edit = new class {
	...
	init({form_id, templateid, thresholds, highlights, regexes, colors, item_value_type, multiselect_item_name}) {
		...
		this.#regexes_table = document.getElementById('regexes_table');
		...
		// Initialize regexes table
		$(this.#regexes_table)
			.dynamicRows({
				rows: regexes,
				template: '#regexes-row-tmpl',
				allow_empty: true
			})
		...
		if (is_item_type_numeric || is_item_type_text) {
			const visible_values = is_item_type_numeric
				...
				: [<?= CWidgetFieldColumnsList::DISPLAY_AS_IS ?>, <?= CWidgetFieldColumnsList::DISPLAY_HTML ?>,
					<?= CWidgetFieldColumnsList::DISPLAY_SINGLE_LINE?>, <?= CWidgetFieldColumnsList::DISPLAY_TEXT_EXTENDED ?>
				];
