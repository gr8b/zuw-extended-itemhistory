...
$table->addItem(
	(new CTemplateTag('regexes-row-tmpl'))
		->addItem((new CRow([
			(new CTextBox('regexes[#{rowNum}][match]', '#{match}', false))
				->addStyle('width: 100%;')
				->setAriaRequired(),
			(new CTextBox('regexes[#{rowNum}][replace]', '#{replace}', false))
				->addStyle('width: 100%;'),
			(new CButton('regexes[#{rowNum}][remove]', _('Remove')))
				->addClass(ZBX_STYLE_BTN_LINK)
				->addClass('element-table-remove')
		]))->addClass('form_row'))
);
...
$form_grid->addItem([
	(new CLabel(_('Regular expressions'), 'regexes_table'))->addClass('js-regexes-row'),
	(new CFormField($table))->addClass('js-regexes-row')
]);
